...
void Editor::draw() {
	...
	std::vector<std::vector<colorpair>> lines{};
	...
	for (int lineNr = scrollY; lineNr < scrollY + getTextEditorHeight() && lineNr < file.linesAmount(); lineNr++) {
		...
		std::vector<colorpair> parsedLine = syntaxHG.parseLine(line, lineNr, this->file.getFileExtension(), *this);
	...
	for (int lineNr = scrollY; lineNr < scrollY + getTextEditorHeight() && lineNr < file.linesAmount(); lineNr++) {
		...
		printw("%3d ", lineNr + 1);
		for (colorpair map : lines[lineNr]) {
			for (int i = 0; i < static_cast<int>(map.size()); i++ ) {
				char symbol = map[i].second;
				int color_pair = map[i].first;
				if (symbol == '\t') {
				...
				if (virtualCol >= scrollX && virtualCol - scrollX < min) {
					...
					printw(std::string{symbol}.c_str());
...
// Defines color pairs		setStatus(this->statusText + "\tinput: " + std::to_string(input), PAIR_STANDARD);
